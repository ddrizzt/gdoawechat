...
echo -e "\n>>>>>>>>>>>>>>>>>>>>>>>>>>>> Deploy oa proxy...\n"
/mnt/wechat-oa-master/deploy_oaproxy_us.sh $ENV

echo "\n>>>>>>>>>>>>>>>>>>>>>>>>>>>> Deploy WeChat mini-programe proxy...\n"
/mnt/wechat-oa-master/deploy_mpproxy.sh

echo -e "\n>>>>>>>>>>>>>>>>>>>>>>>>>>>> Deploy cleanup crontab...\n"
cd /mnt/wechat-oa-master/ && crontab truncatelog_us.cron
